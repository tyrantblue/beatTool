...
BPM 始终指**四分音符**的速度（即 BPM = 每分钟四分音符数）。分母决定节拍器点击的时值相对于四分音符的比例：

```
clickInterval = (60 / BPM) × (4 / denominator)
```

| 拍号 | 120 BPM 下的每拍间隔 | 说明 |
|---|---|---|
| 4/4 | 0.5s | 四分音符 = 一拍，与 BPM 一致 |
| 8/8 | 0.25s | 八分音符 = 半拍，速度为四分音符 2 倍 |
| 16/16 | 0.125s | 十六分音符 = 1/4 拍，速度为四分音符 4 倍 |
| 4/2 | 1.0s | 二分音符 = 2 拍，速度为四分音符一半 |

这样切换拍号时，**整体节奏感不变**（BPM 所定义的四分音符速度恒定），只是点击的细粒度变化。这符合吉他练习的直觉：120BPM 总是同一个"心跳"，拍号只改变你在数什么。
